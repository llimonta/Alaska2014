...
START=$a;   #where you start your frame, the stacking will be centered around this frame, i.e. BegFrame-round(NFRAMES/2):BegFrame+rounds(NFRAMES/2)
...
#		SavedStars="/Stars"$day".mat"; # this is where you save the stars for the days, it is used in the GetMagAndPixelForBrightStars where they are extracted in 						order to be used for the calibration, since it's been done Ntime we locally store them somewhere
		...
		# tUTC in string format "YYYY/MM/DD hh:mm:ss.fff" ss.fff means that we have seconds fractions as well
		echo "Before Matlab"
		...
		if [ $Xpixmin != "NaN" ]&&[ $Ypixmin != "NaN" ]; then 	
			...
			#Hoping/Assuming a succesfull match has been found by solve-fields let's call (don't remember what) which goes on converting each pixel to a
			# txt file
			...
			# Let's now extract all the weird outputted values by CallAstrometry2GrabRaDec into a nice txt matrix file. Remember the first n values are
			# the 
...
kkk=$kkk+1;
echo $kkk
done < /home/limo/PHD/astrometry.net-0.73/astrometry/20140331Events.txt
